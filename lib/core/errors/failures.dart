...
class CameraFailure extends Failure {
  const CameraFailure(super.message, {String? code})
      : super(code: code ?? 'camera_failure');
}

/// Failure when saving a file (e.g. PDF) fails.
class FileSaveFailure extends Failure {
  const FileSaveFailure(super.message, {String? code})
      : super(code: code ?? 'file_save_failure');
}

/// Failure when reading or listing files fails.
class FileReadFailure extends Failure {
  const FileReadFailure(super.message, {String? code})
      : super(code: code ?? 'file_read_failure');
}

/// Failure when opening a file with an external viewer fails.
class FileOpenFailure extends Failure {
  const FileOpenFailure(super.message, {String? code})
      : super(code: code ?? 'file_open_failure');
}

/// Failure when sharing a file via other apps fails.
class ShareFailure extends Failure {
  const ShareFailure(super.message, {String? code})
      : super(code: code ?? 'share_failure');
}

/// Generic unexpected failure for unknown or uncategorized errors.
class UnexpectedFailure extends Failure {
  const UnexpectedFailure(super.message, {String? code})
      : super(code: code ?? 'unexpected_failure');
}

/// Failure when deleting a file fails.
class FileDeleteFailure extends Failure {
  const FileDeleteFailure(super.message, {String? code})
      : super(code: code ?? 'file_delete_failure');
}